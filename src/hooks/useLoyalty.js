...
const useLoyalty = () => {
  ...
  const redeemPoints = useCallback(async (payload) => {
    const result = await loyaltyService.redeemPoints(payload);
    await fetchAccount();
    return result;
  }, [fetchAccount]);
  ...
  const applyTierDiscount = useCallback(async (bookingAmountInRs = 0) => {
    const result = await loyaltyService.applyTierDiscount(bookingAmountInRs);
    await fetchAccount();
