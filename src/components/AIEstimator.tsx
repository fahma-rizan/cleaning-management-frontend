...
import { Calculator, Home, Maximize, Sparkles, ArrowRight } from 'lucide-react';
...
export default function AIEstimator({ onEstimateComplete }: AIEstimatorProps) {
  ...
  const calculateEstimate = () => {
    ...
    if (onEstimateComplete) onEstimateComplete(final, breakdown);
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center justify-center gap-4">
          <button type="button" onClick={() => setFormData({ ...formData, rooms: Math.max(1, formData.rooms - 1) })} className="w-12 h-12 bg-gray-100 rounded-lg hover:bg-gray-200 flex items-center justify-center text-xl">-</button>
          <div className="text-4xl w-20 text-center">{formData.rooms}</div>
          <button type="button" onClick={() => setFormData({ ...formData, rooms: formData.rooms + 1 })} className="w-12 h-12 bg-gray-100 rounded-lg hover:bg-gray-200 flex items-center justify-center text-xl">+</button>
        </div>
      </div>
      <div>
        <label className="block text-lg mb-3">Number of bathrooms</label>
        <div className="flex items-center justify-center gap-4">
          <button type="button" onClick={() => setFormData({ ...formData, bathrooms: Math.max(1, formData.bathrooms - 1) })} className="w-12 h-12 bg-gray-100 rounded-lg hover:bg-gray-200 flex items-center justify-center text-xl">-</button>
          <div className="text-4xl w-20 text-center">{formData.bathrooms}</div>
          <button type="button" onClick={() => setFormData({ ...formData, bathrooms: formData.bathrooms + 1 })} className="w-12 h-12 bg-gray-100 rounded-lg hover:bg-gray-200 flex items-center justify-center text-xl">+</button>
        </div>
      </div>
      <div>
        ...
        <div className="space-y-3">
          <label className="flex items-center gap-3 p-4 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
            <input type="checkbox" checked={formData.hasKitchen} onChange={(e) => setFormData({ ...formData, hasKitchen: e.target.checked })} className="w-5 h-5" />
            <span>Kitchen (+ LKR 800)</span>
          </label>
          <label className="flex items-center gap-3 p-4 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
            <input type="checkbox" checked={formData.hasPets} onChange={(e) => setFormData({ ...formData, hasPets: e.target.checked })} className="w-5 h-5" />
          ...
          {[
            ...
            <button
              ...
              className={`w-full p-4 rounded-xl border-2 text-left transition-all ${
                formData.cleaningLevel === level.id ? 'border-blue-600 bg-blue-50' : 'border-gray-200 hover:border-gray-300'
              }`}
            >
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 rounded-xl p-6">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-gray-600">Base price</span><span>LKR {estimateBreakdown?.basePrice.toLocaleString()}</span></div>
          {estimateBreakdown?.roomExtra > 0 && <div className="flex justify-between"><span className="text-gray-600">Extra rooms</span><span>+ LKR {estimateBreakdown?.roomExtra.toLocaleString()}</span></div>}
          {estimateBreakdown?.bathroomExtra > 0 && <div className="flex justify-between"><span className="text-gray-600">Extra bathrooms</span><span>+ LKR {estimateBreakdown?.bathroomExtra.toLocaleString()}</span></div>}
          {estimateBreakdown?.kitchenExtra > 0 && <div className="flex justify-between"><span className="text-gray-600">Kitchen cleaning</span><span>+ LKR {estimateBreakdown?.kitchenExtra.toLocaleString()}</span></div>}
          {estimateBreakdown?.petExtra > 0 && <div className="flex justify-between"><span className="text-gray-600">Pet-friendly cleaning</span><span>+ LKR {estimateBreakdown?.petExtra.toLocaleString()}</span></div>}
          {estimateBreakdown?.timeMultiplier > 1 && <div className="flex justify-between text-orange-600"><span>Intensive cleaning factor</span><span>×{estimateBreakdown?.timeMultiplier}</span></div>}
          <div className="border-t pt-2 mt-2 flex justify-between font-semibold"><span>Total Estimate</span><span>LKR {estimatedPrice?.toLocaleString()}</span></div>
        </div>
      </div>
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800">
        💡 This is an AI-generated estimate. Final price may vary based on actual inspection.
      </div>
    </div>
  );
      ...
      {estimatedPrice === null ? (
        <>
          ...
          <div className="flex gap-3 mt-8">
            {step > 1 && (
              <button type="button" onClick={() => setStep(step - 1)} className="flex-1 bg-gray-100 text-gray-700 py-3 rounded-lg hover:bg-gray-200 transition-colors">
                Back
              </button>
            )}
            <button
        ...
        <>
          ...
          <button
            type="button"
            onClick={() => { setEstimatedPrice(null); setStep(1); }}
