...
import { Star, Clock, Check, MessageCircle } from 'lucide-react';
...
export default function ServiceDetails({ user, onLogout, theme, onToggleTheme }: ServiceDetailsProps) {
  ...
  const service = {
    ...
    image: isCurtainService
      ? 'https://images.unsplash.com/photo-1660226817472-2c73d49c6dd3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=1080'
      : 'https://images.unsplash.com/photo-1581578949510-fa7315c4c350?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=1080',
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
            {/* Image */}
            <div>
              <img src={service.image} alt={service.name} className="w-full h-96 object-cover rounded-xl shadow-lg" />
            </div>
              ...
              {service.packages.map((pkg) => (
                <div
                  ...
                  className={`bg-white rounded-xl p-6 cursor-pointer transition-all ${
                    selectedPackage === pkg.id ? 'ring-2 ring-purple-600 shadow-lg' : 'hover:shadow-md'
                  } ${(pkg as any).recommended ? 'relative' : ''}`}
                  onClick={() => setSelectedPackage(pkg.id)}
                >
                  {(pkg as any).recommended && (
                    <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
                      <span className="bg-purple-600 text-white px-3 py-1 rounded-full text-sm">Recommended</span>
                    </div>
                  )}
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              <Link to={`/reviews/${service.id}`} className="text-purple-600 hover:text-purple-700 flex items-center gap-2">
