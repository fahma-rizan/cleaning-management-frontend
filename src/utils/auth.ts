...
export const tokenStorage = {
  ...
  getTokens: (): AuthTokens | null => {
    ...
    // Backwards compatibility: some older code stored a single token under
    // the key `token`. If that's present, treat it as the access token.
    const legacyToken = localStorage.getItem('token');

    // If we have an access token, return it even if refreshToken is missing.
    // This avoids returning null and silently omitting Authorization headers
    // in places that only need the access token (common in this codebase).
    if (accessToken) {
      return { accessToken, refreshToken: refreshToken || '' };
    }

    if (legacyToken) {
      return { accessToken: legacyToken, refreshToken: refreshToken || '' };
    }
  ...
  getAccessToken: (): string | null => {
    // Prefer the new `accessToken` key but fall back to legacy `token`.
  ...
  async apiCall(endpoint: string, options: RequestInit = {}): Promise<Response> {
    ...
    if (response.status === 401) {
      ...
      if (refreshed) {
        ...
        return fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}${endpoint}`, {
          ...options,
          headers,
        });
      }
