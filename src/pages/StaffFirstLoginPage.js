      ...
      <div style={{ position: 'relative' }}>
        <input
          ...
          style={{
            ...
            background: focused ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.06)',
            border: focused ? '1px solid rgba(16,185,129,0.6)' : '1px solid rgba(255,255,255,0.1)',
...
const StaffFirstLoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const strengthError = strength.rules.find(r => !r.test);
    if (strengthError) {
      setError(`Password too weak — ${strengthError.msg.toLowerCase()} required.`);
      return;
    }
  ...
  return (
    <div style={{
      ...
      background: 'linear-gradient(135deg, #0d0d1a 0%, #0d1f15 40%, #0d0d1a 100%)',
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <BadgeIcon sx={{ fontSize: 26, color: '#fff' }} />
          </div>
          <h1 style={{ color: '#fff', fontWeight: 700, fontSize: 20, margin: 0 }}>Welcome to the Team!</h1>
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13, margin: '6px 0 0' }}>
            Hi {user?.name?.split(' ')[0]}! You must set a personal password before you can continue.
          </p>
        </div>
        ...
        <div style={{ padding: '28px 36px 32px' }}>
          {success ? (
            <div style={{ textAlign: 'center', padding: '16px 0' }}>
              ...
              <p style={{ color: '#10B981', fontWeight: 700, fontSize: 15, margin: 0 }}>Password updated successfully!</p>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13, marginTop: 6 }}>Redirecting to your dashboard…</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 12px', marginTop: 8 }}>
                      {strength.rules.map(r => (
                        <span key={r.msg} style={{ fontSize: 11, color: r.test ? '#10B981' : 'rgba(255,255,255,0.3)' }}>
                          {r.test ? '✓' : '○'} {r.msg}
                        </span>
                      ))}
                    </div>
              ...
              <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.25)', fontSize: 11.5, margin: 0 }}>
                You cannot access your dashboard until this step is complete.
              </p>
            </form>
          )}
        </div>
...
export default StaffFirstLoginPage;
