      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ background: '#f9fafb' }}>
            ...
            <th style={{ ...thStyle, textAlign: 'right' }}>Restock Cost</th>
              ...
              <tr key={i} style={{ background: i % 2 === 0 ? '#ffffff' : '#f9fafb' }}>
                ...
                <td style={{ ...tdStyle, textAlign: 'right' }}>
                  {row.restockCost != null
                    ? `LKR ${Number(row.restockCost).toLocaleString('en-LK', { minimumFractionDigits: 2 })}`
                    : '—'}
                </td>
  ...
  const downloadPDF = () => {
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ['Restock Cost',    totals.totalRestockCost != null
          ? `LKR ${Number(totals.totalRestockCost).toLocaleString('en-LK', { minimumFractionDigits: 2 })}`
          : 'LKR 0.00'],
      ],
      styles: { fontSize: 10 },
    ...
    if (breakdown.length > 0) {
      ...
      autoTable(doc, {
        ...
        head: [['Item', 'SKU', 'Type', 'Deducted', 'Returned', 'Net Used', 'Restocked', 'Cost', 'Txns']],
        ...
        <>
          ...
          <section>
            ...
            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
              ...
              <StatCard
                label="Restock Cost"
                value={totals.totalRestockCost != null
                  ? `LKR ${Number(totals.totalRestockCost).toLocaleString('en-LK', { minimumFractionDigits: 2 })}`
                  : '0'}
                color="#F59E0B"
              />
